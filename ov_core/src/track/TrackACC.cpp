...
void TrackACC::feed_stereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right) {
  ...
  cv::Mat disparity = message.disparities[0];
  ...
  if (pts_last[cam_id_left].empty() && pts_last[cam_id_right].empty()) {
    ...
    perform_detection_stereo(imgpyr_left, imgpyr_right, mask_left, mask_right, disparity, cam_id_left, cam_id_right, good_left, good_right,
  ...
  perform_detection_stereo(img_pyramid_last[cam_id_left], img_pyramid_last[cam_id_right], img_mask_last[cam_id_left],
                           img_mask_last[cam_id_right], disparity, cam_id_left, cam_id_right, pts_left_old, pts_right_old, ids_left_old,
                           ids_right_old);
  ...
  parallel_for_(cv::Range(0, 2), LambdaBody([&](const cv::Range &range) {
                  for (int i = range.start; i < range.end; i++) {
                    ...
                    perform_matching(img_pyramid_last[is_left ? cam_id_left : cam_id_right], is_left ? imgpyr_left : imgpyr_right,
                                     ...
                                     is_left ? mask_ll : mask_rr, disparity);
                  }
                }));
...
void TrackACC::perform_detection_stereo(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr, const cv::Mat &mask0,
  ...
  if (num_featsneeded_0 > std::min(20, (int)(min_feat_percent * num_features))) {
    ...
    if (!pts0_new.empty()) {
      ...
      //std::cout << "disparity size --> : " << disp.rows << " = " << disp.cols << std::endl;
      /*
      cv::Mat real_disp;
      disp.convertTo(real_disp, CV_32FC1, 1.0 / 256.0);

        cv::Mat disp_norm, disp_color;

        cv::Mat valid_mask = (real_disp > 0) & (real_disp < 192.0);
        real_disp.setTo(0, ~valid_mask);


        double max_val, min_val;
        cv::minMaxLoc(disp, &min_val, &max_val, nullptr, nullptr, valid_mask);
        std::cout << "max mine: " << max_val << " x " << min_val << std::endl;

        // Step 2: Normalize (bright = close)
        disp.convertTo(disp_norm, CV_8UC1, -255.0 / (max_val - min_val), 255.0 * max_val / (max_val - min_val));

        // Step 3: Apply perceptually uniform colormap
        cv::applyColorMap(disp_norm, disp_color, cv::COLORMAP_MAGMA);

        cv::imshow("Left + Disparity", disp_color);
        cv::waitKey(1);
      */

      std::vector<uchar> mask;
      perform_stereo_matching(img0pyr, img1pyr, disp, pts0_new, pts1_new, cam_id_left, cam_id_right, mask);
      ...
      //                         cv::OPTFLOW_USE_INITIAL_FLOW);
       ...
       /*
       std::cout << "sssssssssssssssssssss: " << pts0_new.size() << " = " << pts1_new.size() << std::endl;
...
void TrackACC::perform_stereo_matching(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr,
                                       const cv::Mat &disp, std::vector<cv::Point2f> &pts0, std::vector<cv::Point2f> &pts1,
  ...
  std::cerr << "Using disparity ------------------------------------------------!" << std::endl;
  mask_out.clear();
  for (size_t i = 0; i < pts0.size(); ++i) {
    int x = static_cast<int>(pts0[i].x);
    int y = static_cast<int>(pts0[i].y);
    if (x >= 0 && x < real_disp.cols && y >= 0 && y < real_disp.rows) {
      float d = real_disp.at<float>(y, x);
      if (d > 0) {
        pts1[i].x = pts0[i].x - d;
        pts1[i].y = pts0[i].y;
  ...
  //cv::findFundamentalMat(pts0, pts1, cv::FM_RANSAC, 2.0 / max_focallength, 0.999, mask_rsc);
...
void TrackACC::perform_matching(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr, std::vector<cv::KeyPoint> &kpts0,
                                std::vector<cv::KeyPoint> &kpts1, size_t id0, size_t id1, std::vector<uchar> &mask_out, const cv::Mat &disp) {
  ...
  //std::vector<uchar> mask_klt;
  //perform_stereo_matching(img0pyr, img1pyr, disp, pts0, pts1, id0, id1, mask_out);
...
void TrackACC::vis_stereo(const std::vector<cv::Point2f> &pts0,
